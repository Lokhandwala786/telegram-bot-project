...
async def poll_once(settings: Settings) -> int:
    ...
    async def send_job_alert(job: JobListing, text: str) -> None:
        nonlocal sent
        if settings.dry_run:
            log.info("[DRY-RUN] Would send Telegram job alert:\n%s", text)
            sent += 1
            return
        assert token
        for cid in chat_ids:
